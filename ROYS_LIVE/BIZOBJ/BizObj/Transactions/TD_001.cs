...
namespace BizObj
{
    public class TD_001 : BizObj.IGetData
    {
       ...
       public string GetData(string Xml, string AddParams, string Mode)
       {
           ...
           try
           {
               if (Mode == "S")
               {
                   ...
                   return PLABS .Utils .CnvDataSetToXml( objDbHelper.SelectSP("display_sel", Xml, 1, this._objDb),true);

               }
               else if (Mode == "I" || Mode == "U")
               {
                   PLABS.DAL objDbHelper = new PLABS.DAL();
                   return objDbHelper.insertSP("display_iu", Xml, this._objDb);
               }
               else if (Mode == "D")
               {
                   PLABS.DAL objDbHelper = new PLABS.DAL();
                   return objDbHelper.insertSP("display_del", Xml, this._objDb);
               }
           }
           catch (Exception ex)
           {
