...
namespace BizObj
{
    public class TS_002 : BizObj.IGetData
	{
		...
		public string GetData(string Xml,  string AddParams,string Mode)
		{
			...
			try
			{
				if (Mode == "S")
				{
					...
					return PLABS .Utils .CnvDataSetToXml(objDbHelper.SelectSP("trsales_SEL", Xml, 1, this._objDb),true);

				}
				else if (Mode == "I" || Mode == "U"  )
				{
					PLABS.DAL objDbHelper = new PLABS.DAL();
                    return objDbHelper.insertSP("trsales_iu", Xml, this._objDb);
				}
				else if (Mode == "D")
				{
					PLABS.DAL objDbHelper = new PLABS.DAL();
					return objDbHelper.insertSP("trsales_DEL", Xml, this._objDb);
				}
