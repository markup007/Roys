...
namespace BizObj
{
    public class CO_003 : BizObj.IGetData
	{
		...
		public string GetData(string Xml,  string AddParams,string Mode)
		{
			...
			try
			{
				if (Mode == "S")
				{
                    ...
                    return PLABS .Utils .CnvDataSetToXml (objDbHelper.SelectSP("pettycashsheet_sel", Xml, 1, this._objDb),true);
				...
				{
					PLABS.DAL objDbHelper = new PLABS.DAL();
                    return objDbHelper.insertSP("pettycashsheet_del", Xml, this._objDb);
				}
			}
			catch (Exception ex)
			{
