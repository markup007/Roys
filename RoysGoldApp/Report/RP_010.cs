...
namespace RoysGold
{   public partial class RP_010 : PLABS.MasterForm
    {
       ...
       // double pur_wt = 0;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            this.fnd_shop_id.AfterFind += new EventHandler(fnd_shop_id_AfterFind);
            base.OnLoad(e);
        }
        ...
        void fnd_shop_id_AfterFind(object sender, EventArgs e)
        {
            try
            {
                this.LoadReport();
        ...
        private void loadControls()
        {
            try
            {
                ...
                dp_frm.ControlValue = PLABS .Utils .CnvToDate(string.Format("{0}-01-{1}",DateTime .Now.Month.ToString(),DateTime .Now.Year.ToString())).ToString("dd-MMM-yyyy");
                
                dp_to.ControlValue = (PLABS .Utils.CnvToDate(dp_frm.ControlValue).AddMonths(1)).AddDays(-1).ToString("dd-MMM-yyyy");

               
                DataSet ds = this.GetDataSet("LG", (dp_frm.Value.ToString("yyyy_MM_dd")), (dp_to.Value.ToString("yyyy_MM_dd")),this.fnd_gold_id .ControlValue,this.fnd_shop_id.ControlValue);

                this.fnd_shop_id.LoadData(ds.Tables[0], "cd", "nam", "id");
                this.fnd_gold_id.LoadData(ds.Tables[1], "grp", "grp", "id");
                this.Print(ds);
        ...
        //    }
        //    }
        private String getSelectArgs(String as_mode, String as_frm_date, String as_to_date, String ai_itm_id, String ai_addr_id)
        {
            try
            {
        ...
        private void Print(DataSet ds)
        {
            try
            {
                ...
                this.rtxt_purchase .Text =this.GetFirstPrintString(ds.Tables[0], ds.Tables[2]);
            }
            catch (Exception ex)
            {
        ...
        private DataSet GetDataSet(String as_mode, String as_frm_date, String as_to_date, String ai_itm_id, String ai_addr_id)
        {
            DataSet ds = new DataSet();
            try
            {
                string xmlData = this.getBlankXml();
                xmlData = PLABS.Utils.addNodes(xmlData, getSelectArgs(as_mode, as_frm_date, as_to_date, ai_itm_id,ai_addr_id));
                xmlData = this.CallWS(xmlData, "BizObj.RP_010,BizObj", "S");
        ...
        private String GetFirstPrintString(DataTable master, DataTable detail)
        {
            try
            {
                ...
                for (int i = 0; i < master.Rows.Count; i++)
                {
                    ...
                    string itemName = PLABS.Utils.CnvToStr(master.Rows[i]["nam"]);
