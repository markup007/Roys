...
namespace RoysGold
{
    public partial class RP_008 : PLABS.MasterForm
    {
        ...
        private void loadControls()
        {
            try
            {
                ...
                DataSet ds = this.GetDataSet("LG",dtFrom.ToString("yyyy-MM-dd"),dtTo);


                if (ds.Tables.Count != 0)
                {
                    DataRow[] dr = PLABS.Utils.GetDataTable(ds, 0).Select("vtyp='O' AND name='OPENING'");
                 
                    if (dr.Length != 0)
                    {
                        _tropn = PLABS.Utils.CnvToDouble(dr[0]["wt"]);
                    }
                    DataRow[] dr1 = PLABS.Utils.GetDataTable(ds, 0).Select("vtyp='CO' AND name='OPENING AMOUNT'");
                    if (dr1.Length != 0)
                    {
                        _tropnAmt = PLABS.Utils.CnvToDouble(dr1[0]["wt"]);
                    }
                    if (PLABS .Utils.GetDataTable(ds,2).Rows.Count != 0)
                    {
                        this.txt_opnwt.Text = PLABS.Utils.CnvToDouble(PLABS.Utils.GetDataTable(ds, 2).Rows[0]["opnWt"]).ToString("F3");
                        this.txt_opnAmt.Text = PLABS.Utils.CnvToDouble(PLABS.Utils.GetDataTable(ds, 2).Rows[0]["opnAmt"]).ToString("F2");
                        this.txt_brdrt.Text = PLABS.Utils.CnvToDouble(PLABS.Utils.GetDataTable(ds, 2).Rows[0]["brd_rt"]).ToString("F2");
                    }
                    if (PLABS.Utils.GetDataTable(ds, 3).Rows.Count != 0)
                    {
                        _trsaleopn = PLABS.Utils.CnvToDouble(PLABS.Utils.GetDataTable(ds, 3).Rows[0]["salopn"]);
                    }
        ...
        private void loadData(DataTable data)
        {
            ...
            foreach (DataRow dr in data.Rows)
            {
                ...
                else if (PLABS.Utils.CnvToStr(dr["vtyp"]) == "WP" || PLABS.Utils.CnvToStr(dr["vtyp"]) == "WO")
                {
        ...
        private DataSet GetDataSet(String as_mode, String ai_frm, String ai_to)
        {
            try
            {
                ...
                xmlData = this.CallWS(xmlData, "BizObj.RP_008,BizObj", "S");
        ...
        private void MontlyReport(int rowNum)
        {
            try
            {
                ...
                _yr = PLABS.Utils.CnvToDate(date).Year;
