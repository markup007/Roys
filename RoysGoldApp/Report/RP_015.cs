...
namespace RoysGold
{
    public partial class RP_015 : PLABS.MasterForm
    {
        ...
        {
            if (this.txt_brdrt.ControlValue == string.Empty)
            {
                PLABS.MessageBoxPL.Show("Enter Board Rate", "Alert", PLABS.MessageBoxPL.PLMessageBoxButtons.OK, PLABS.MessageBoxPL.PLMessageBoxIcon.Warning);
                txt_brdrt.Focus();
            }
            else if (txt_amt.ControlValue == string.Empty)
            {
                PLABS.MessageBoxPL.Show("Please Enter Fixed Amount", "Alert", PLABS.MessageBoxPL.PLMessageBoxButtons.OK, PLABS.MessageBoxPL.PLMessageBoxIcon.Warning);
                txt_amt.Focus();
            }
            else if (this.grd_gldrgtr.DataSource == null || ((DataTable)this.grd_gldrgtr.DataSource).Rows.Count == 0)
            {
                PLABS.MessageBoxPL.Show("No Detail", "Alert", PLABS.MessageBoxPL.PLMessageBoxButtons.OK, PLABS.MessageBoxPL.PLMessageBoxIcon.Warning);
            }
            else
            {
                doSave();
        ...
        void btn_curmnth_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txt_brdrt.ControlValue == string.Empty)
                {
                    PLABS.MessageBoxPL.Show("Please Enter Board Rate", "Alert", PLABS.MessageBoxPL.PLMessageBoxButtons.OK, PLABS.MessageBoxPL.PLMessageBoxIcon.Warning);
        ...
        private void loadControls()
        {
            try
            {
                ...
                {
                    this.txt_brdrt.ControlValue = PLABS.Utils.CnvToStr(ds.Tables[0].Rows[0]["bd_rate"]);
                    this.txt_amt.ControlValue = PLABS.Utils.CnvToStr(ds.Tables[0].Rows[0]["fix_amt"]);
        ...
        private void loadGrid()
        {
            ...
            if (ds.Tables.Count != 0)
            {
                ...
                DataTable dt2 = PLABS.Utils.GetDataTable(ds, 2);

                this.loadData(dt, dt1, dt2);
                //this.grd_cptl.DataSource = PLABS.Utils.GetDataTable(ds, 1);
            }
        ...
        private void loadData(DataTable data,DataTable dt1,DataTable dt2)
        {
            ...
            {
                openingAmount = Math.Round((bal * boardRate), 2);
                dar["wt"] = openingAmount;
                dar["bala"] = openingAmount;
            ...
            foreach (DataRow dr1 in dt1.Rows)
            {
                ...
                {
                    DataRow newRow = dt.NewRow();
                    cbaln += Math.Round(PLABS.Utils.CnvToDouble(dr1["wt"]),2);

                    newRow["name"] = dr1["name"];

                    newRow["wt"] = Math.Round(PLABS.Utils.CnvToDouble(dr1["wt"]), 2);
                    newRow["bala"] = cbaln;
                ...
                else
                {
                    ...
                    drCashBalance["wt"] = Math.Round(PLABS.Utils.CnvToDouble(dr1["wt"]), 2);
            ...
            {
                cbaln += Math.Round(PLABS.Utils.CnvToDouble(drCashBalance["wt"]), 2);
        ...
        private DataSet GetDataSet(String as_mode, String ai_frm, String ai_to)
        {
            try
            {
                ...
                xmlData = this.CallWS(xmlData, "BizObj.RP_015,BizObj", "S");
        ...
        private String getSelectArgs(String as_mode,String ai_frm,String ai_to)
        {
            try
            {
                ...
                //argXml = PLABS.Utils.addNode(argXml, "ai_opnwt", PLABS.Utils.CnvToDouble(this.txt_opnwt.Text).ToString("F3"));
                //argXml = PLABS.Utils.addNode(argXml, "ai_opnamt", PLABS.Utils.CnvToDouble(this.txt_opnAmt.Text).ToString("F2"));
                argXml = PLABS.Utils.addNode(argXml, "ai_brdrt", PLABS.Utils.CnvToDouble(this.txt_brdrt.Text).ToString("F2"));
                argXml = PLABS.Utils.addNode(argXml, "ai_fix_amt", PLABS.Utils.CnvToDouble(this.txt_amt.Text).ToString("F2"));
        ...
        private void doSave()
        {
            try
            {
                ...
                xmlData = this.CallWS(xmlData, "BizObj.RP_015,BizObj", "I");
                if (xmlData == "-1")
                {
